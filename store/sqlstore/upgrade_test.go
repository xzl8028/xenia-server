...
func TestStoreUpgrade(t *testing.T) {
	StoreTest(t, func(t *testing.T, ss store.Store) {
		...
		t.Run("upgrade from unsupported version", func(t *testing.T) {
			...
			err := UpgradeDatabase(sqlStore, "5.8.0")
			require.EqualError(t, err, "Database schema version 2.0.0 is no longer supported. This Xenia server supports automatic upgrades from schema version 3.0.0 through schema version 5.8.0. Please manually upgrade to at least version 3.0.0 before continuing.")
		...
		t.Run("upgrade schema running later major version", func(t *testing.T) {
			...
			err := UpgradeDatabase(sqlStore, "5.8.0")
			require.EqualError(t, err, "Database schema version 6.0.0 is not supported. This Xenia server supports only >=5.8.0, <6.0.0. Please upgrade to at least version 6.0.0 before continuing.")
