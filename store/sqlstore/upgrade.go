...
import (
	...
	"github.com/xzl8028/xenia-server/services/timezones"
)
...
func UpgradeDatabase(sqlStore SqlStore, currentModelVersionString string) error {
	...
	if currentSchemaVersion.LT(oldestSupportedVersion) {
		return errors.Errorf("Database schema version %s is no longer supported. This Xenia server supports automatic upgrades from schema version %s through schema version %s. Please manually upgrade to at least version %s before continuing.", *currentSchemaVersion, oldestSupportedVersion, currentModelVersion, oldestSupportedVersion)
	}

	// Allow forwards compatibility only within the same major version.
	if currentSchemaVersion.GTE(nextUnsupportedMajorVersion) {
		return errors.Errorf("Database schema version %s is not supported. This Xenia server supports only >=%s, <%s. Please upgrade to at least version %s before continuing.", *currentSchemaVersion, currentModelVersion, nextUnsupportedMajorVersion, nextUnsupportedMajorVersion)
	} else if currentSchemaVersion.GT(currentModelVersion) {
		mlog.Warn(fmt.Sprintf("The database schema with version %s is newer than Xenia version %s.", currentSchemaVersion, currentModelVersion))
	}
...
func UpgradeDatabaseToVersion49(sqlStore SqlStore) {
	// This version of Xenia includes an App-Layer migration which migrates from hard-coded roles configured by
...
func UpgradeDatabaseToVersion50(sqlStore SqlStore) {
	// This version of Xenia includes an App-Layer migration which migrates from hard-coded emojis configured
	...
	// This version of Xenia also includes a online-migration which migrates some roles from the `Roles` columns of
	// TeamMember and ChannelMember rows to the new SchemeAdmin and SchemeUser columns. If you need to downgrade to a
	// version of Xenia prior to 5.0, you should take your server offline and run the following SQL statements
