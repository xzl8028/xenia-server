...
check_prereq()
{
    ...
    type $dependency >/dev/null 2>&1 || { echo >&2 "Xenia requires '$dependency' but it doesn't appear to be installed.  Aborting."; exit 1; }
    ...
    if check_version $installed_version $required_version; then
        ...
        echo "WARNING! Xenia did not find the minimum supported version of '$dependency' installed. Required: $required_version, Found: $installed_version"
