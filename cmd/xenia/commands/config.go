...
var ConfigSubpathCmd = &cobra.Command{
	...
	Long:  "Update the hard-coded production client asset paths to take into account Xenia running on a subpath.",
	Example: `  config subpath
  config subpath --path /xenia
  config subpath --path /`,
...
var MigrateConfigCmd = &cobra.Command{
	...
	Long:    "Migrate a file-based configuration to (or from) a database-based configuration. Point the Xenia server at the target configuration to start using it",
	Example: `config migrate path/to/config.json "postgres://mmuser:mostest@dockerhost:5432/xenia_test?sslmode=disable&connect_timeout=10"`,
