...
func randomMessage(users []string) string {
	...
	case 1:
		switch rand.Intn(2) {
		case 0:
			xeniaVideos := []string{"Q4MgnxbpZas", "BFo7E9-Kc_E", "LsMLR-BHsKg", "MRmGDhlMhNA", "mUOPxT7VgWc"}
			message = "https://www.youtube.com/watch?v=" + xeniaVideos[rand.Intn(len(xeniaVideos))]
		case 1:
			xeniaTweets := []string{"943119062334353408", "949370809528832005", "948539688171819009", "939122439115681792", "938061722027425797"}
			message = "https://twitter.com/xeniahq/status/" + xeniaTweets[rand.Intn(len(xeniaTweets))]
		}
	case 2:
		message = ""
		if rand.Intn(2) == 0 {
...
func sampleDataCmdF(command *cobra.Command, args []string) error {
	...
	switch bulk {
	case "":
		bulkFile, err = ioutil.TempFile("", ".xenia-sample-data-")
...
func createUser(idx int, teamMemberships int, channelMemberships int, teamsAndChannels map[string][]string, profileImages []string, guest bool) app.LineImportData {
	...
	email := fmt.Sprintf("user-%d@sample.xenia.com", idx)
	if guest {
		password = fmt.Sprintf("guest-%d", idx)
		email = fmt.Sprintf("guest-%d@sample.xenia.com", idx)
	}
	...
	if guest {
		if idx == 0 {
			username = "guest"
			password = "guest"
			email = "guest@sample.xenia.com"
		}
	} else if idx == 0 {
		username = "sysadmin"
		password = "sysadmin"
		email = "sysadmin@sample.xenia.com"
	} else if idx == 1 {
		username = "user-1"
	}
