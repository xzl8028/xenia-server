...
func init() {
	...
	MigrateAuthCmd.Flags().Bool("auto", false, "Automatically migrate all users. Assumes the usernames and emails are identical between Xenia and SAML services. (saml only)")
  ...
  xenia user migrate_auth [from_auth] [to_auth] [migration-options] [flags]
...
Arguments:
  ...
  migration-options (saml):
    users_file:
      The path of a json file with the usernames and emails of all users to migrate to SAML. The username and email must be the same that the SAML service provider store. And the email must match with the email in xenia database.
...
func inviteUser(a *app.App, email string, team *model.Team, teamArg string) error {
	...
	a.SendInviteEmails(team, "Administrator", "Xenia CLI "+model.NewId(), invites, *a.Config().ServiceSettings.SiteURL)
...
func migrateAuthToLdapCmdF(command *cobra.Command, args []string) error {
	...
	// Email auth in Xenia system is represented by ""
	if fromAuth == "email" {
		fromAuth = ""
	}
...
func migrateAuthToSamlCmdF(command *cobra.Command, args []string) error {
	...
	if autoFlag && !dryRunFlag {
		var confirm string
		CommandPrettyPrintln("You are about to perform an automatic \"" + fromAuth + " to saml\" migration. This must only be done if your current Xenia users with " + fromAuth + " auth have the same username and email in your SAML service. Otherwise, provide the usernames and emails from your SAML Service using the \"users file\" without the \"--auto\" option.\n\nDo you want to proceed with automatic migration anyway? (YES/NO):")
		fmt.Scanln(&confirm)
