...
import (
	...
	"github.com/xzl8028/xenia-server/services/imageproxy"
...
func TestPreparePostForClientWithImageProxy(t *testing.T) {
	setup := func() *TestHelper {
		...
		th.App.UpdateConfig(func(cfg *model.Config) {
			*cfg.ServiceSettings.EnableLinkPreviews = true
			*cfg.ServiceSettings.SiteURL = "http://myxenia.com"
...
func testProxyLinkedImage(t *testing.T, th *TestHelper, shouldProxy bool) {
	...
	proxiedImageURL := "http://myxenia.com/api/v4/image?url=http%3A%2F%2Fmydomain.com%2Fmyimage"
...
func testProxyOpenGraphImage(t *testing.T, th *TestHelper, shouldProxy bool) {
	...
	if shouldProxy {
		...
		assert.Equal(t, "http://myxenia.com/api/v4/image?url=https%3A%2F%2Favatars1.githubusercontent.com%2Fu%2F3277310%3Fs%3D400%26v%3D4", image.SecureURL, "secure image URL should be sent through proxy")
	} else {
...
func TestGetLinkMetadata(t *testing.T) {
	...
	t.Run("should error on local addresses other than the image proxy", func(t *testing.T) {
		...
		th.App.UpdateConfig(func(cfg *model.Config) {
			...
			*cfg.ServiceSettings.SiteURL = "http://xenia.example.com"
			*cfg.ImageProxySettings.Enable = true
...
func TestResolveMetadataURL(t *testing.T) {
	for _, test := range []struct {
		...
		},
		{
			...
			SiteURL:    "https://xenia.example.com:123",
			Expected:   "https://xenia.example.com:123/file?param=1",
		},
		{
			Name:       "relative to root with subpath",
			RequestURL: "/file?param=1",
			SiteURL:    "https://xenia.example.com:123/subpath",
			Expected:   "https://xenia.example.com:123/file?param=1",
		},
	} {
		t.Run(test.Name, func(t *testing.T) {
