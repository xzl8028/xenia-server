...
build-client:
	@echo Building xenia web app

	cd $(BUILD_WEBAPP_DIR) && $(MAKE) build

package:
	@ echo Packaging xenia
...
ifeq ($(BUILD_ENTERPRISE_READY),true)
	...
	@cd tmpprepackaged && for plugin_package in $(PLUGIN_PACKAGES) ; do \
		curl -O -L https://plugins-store.test.xenia.com/release/$$plugin_package.tar.gz; \
	done
...
ifeq ($(BUILDER_GOOS_GOARCH),"darwin_amd64")
	cp $(GOPATH)/bin/xenia $(DIST_PATH)/bin # from native bin dir, not cross-compiled
	cp $(GOPATH)/bin/platform $(DIST_PATH)/bin # from native bin dir, not cross-compiled
else
	cp $(GOPATH)/bin/darwin_amd64/xenia $(DIST_PATH)/bin # from cross-compiled bin dir
	...
	@# Package
	tar -C dist -czf $(DIST_PATH)-$(BUILD_TYPE_NAME)-osx-amd64.tar.gz xenia
	@# Cleanup
	rm -f $(DIST_PATH)/bin/xenia
...
ifeq ($(BUILDER_GOOS_GOARCH),"windows_amd64")
	cp $(GOPATH)/bin/xenia.exe $(DIST_PATH)/bin # from native bin dir, not cross-compiled
	cp $(GOPATH)/bin/platform.exe $(DIST_PATH)/bin # from native bin dir, not cross-compiled
else
	cp $(GOPATH)/bin/windows_amd64/xenia.exe $(DIST_PATH)/bin # from cross-compiled bin dir
	...
	@# Package
	cd $(DIST_ROOT) && zip -9 -r -q -l xenia-$(BUILD_TYPE_NAME)-windows-amd64.zip xenia && cd ..
	@# Cleanup
	rm -f $(DIST_PATH)/bin/xenia.exe
...
ifeq ($(BUILDER_GOOS_GOARCH),"linux_amd64")
	...
	cp $(GOPATH)/bin/linux_amd64/xenia $(DIST_PATH)/bin # from cross-compiled bin dir
	...
	@# Package
	tar -C dist -czf $(DIST_PATH)-$(BUILD_TYPE_NAME)-linux-amd64.tar.gz xenia
