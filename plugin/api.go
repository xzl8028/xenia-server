...
//
// Plugins obtain access to the API by embedding XeniaPlugin and accessing the API member
// directly.
type API interface {
	...
	// GetLicense returns the current license used by the Xenia server. Returns nil if the
	// the server does not have a license.
	//
	// Minimum server version: 5.10
	GetLicense() *model.License

	// GetServerVersion return the current Xenia server version
	//
	// Minimum server version: 5.4
	GetServerVersion() string

	// GetSystemInstallDate returns the time that Xenia was first installed and ran.
	//
	// Minimum server version: 5.10
	...
	// payload is the data sent with the event. Interface values must be primitive Go types or xenia-server/model types.
	...
	// LogDebug writes a log message to the Xenia server log file.
	...
	// LogInfo writes a log message to the Xenia server log file.
	// Appropriate context such as the plugin name will already be added as fields so plugins
	// do not need to add that info.
	// keyValuePairs should be primitive go types or other values that can be encoded by encoding/gob
	LogInfo(msg string, keyValuePairs ...interface{})

	// LogError writes a log message to the Xenia server log file.
	// Appropriate context such as the plugin name will already be added as fields so plugins
	// do not need to add that info.
	// keyValuePairs should be primitive go types or other values that can be encoded by encoding/gob
	LogError(msg string, keyValuePairs ...interface{})

	// LogWarn writes a log message to the Xenia server log file.
	// Appropriate context such as the plugin name will already be added as fields so plugins
	// do not need to add that info.
	// keyValuePairs should be primitive go types or other values that can be encoded by encoding/gob
