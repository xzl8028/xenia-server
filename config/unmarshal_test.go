...
func TestUnmarshalConfig_PluginSettings(t *testing.T) {
	...
	assert.Equal(t, "/temp/xenia-plugins", *config.PluginSettings.Directory)
...
func TestConfigFromEnviroVars(t *testing.T) {
	config := `{
		...
		"SupportSettings": {
			"TermsOfServiceLink": "https://about.xenia.com/default-terms/"
		},
		"PluginSettings": {
			"Enable": true,
			"Plugins": {
	...
	t.Run("string settings", func(t *testing.T) {
		...
		assert.Equal(t, "Xenia", *cfg.TeamSettings.SiteName)
